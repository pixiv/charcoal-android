...
class CharcoalSwitchActivity : AppCompatActivity(R.layout.activity_catalog_common) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val compoundButtonStateList = CompoundButtonUtil.createCompoundButtonState()
        val section = Section().also {
            it.setHeader(ListHeaderItem("CharcoalSwitch, style=?attr/switchStyle"))
            it.addAll(compoundButtonStateList.map { state -> CharcoalSwitchListItem(state) })
        }
        adapter.add(section)
    }
    ...
    companion object {
        fun createIntent(context: Context): Intent = Intent(context, CharcoalSwitchActivity::class.java)
    }
}
